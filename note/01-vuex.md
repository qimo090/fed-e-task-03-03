...
![state-view-actions](https://tva1.sinaimg.cn/large/007S8ZIlgy1ghi8v82wkqj30zk0o2wfx.jpg)

# 组件间通信方式回顾

大多数场景下的组件都并不是独立存在的，而是相互协作共同构成了一个复杂的业务功能。在 Vue 中为 不同的组件关系提供了不同的通信规则。

![communication](https://tva1.sinaimg.cn/large/007S8ZIlgy1ghi8w6i7buj30gc0d4my3.jpg)
...
- 首先创建一个共享的仓库 store 对象
  ...
  export default {
    debug: true,
    state: { user: { name: 'xiaomao', age: 18, sex: '男' } },
...
export default {
  ...
  data() {
    return { privateState: {}, sharedState: store.state }
  },
}
```
...
>
> 如果您不打算开发大型单页应用，使用 Vuex 可能是繁琐冗余的。确实是如此——如果您的应用 够简单，您最好不要使用 Vuex。一个[简单的 store 模式](https://cn.vuejs.org/v2/guide/state-management.html#%E7%AE%80%E5%8D%95%E7%8A%B6%E6%80%81%E7%AE%A1%E7%90%86%E8%B5%B7%E6%AD%A5%E4%BD%BF%E7%94%A8)就足够您所需了。但是，如果您需要构建 一个中大型单页应用，您很可能会考虑如何更好地在组件外部管理状态，Vuex 将会成为自然而然 的选择。引用 Redux 的作者 Dan Abramov 的话说就是：Flux 架构就像眼镜：您自会知道什么时 候需要它。
...
![vuex](https://tva1.sinaimg.cn/large/007S8ZIlgy1ghi9mhuow8j30jh0fbwef.jpg)
...
- Vuex 中创建两个模块，分别用来记录商品列表和购物车的状态，store 的结构：
  ...
  ![modules](https://tva1.sinaimg.cn/large/007S8ZIlgy1ghia0nk8rtj307k04s0sv.jpg)
...
<!-
  ...
  @change="updateProductChecked(productId, $event)" 123, 原来那个默认参数 当你传递了自定义参数的时候，还想得到原来那个默认参数，就手动传递一个 $event
-->
...
methods: {
  ...mapMutations('cart', [
    'updateAllProductsChecked', 'updateProductChecked'
  ])
}
```
...
<template slot-scope="scope">
  <el-input-number
    ...
    @change="updateProductCount({ prodId: scope.row.id, count: \$event })"
  ></el-input-number>
</template>
```
...
```js
...mapMutations('cart', [
  'updateAllProductsChecked', 'updateProductChecked', 'updateProductCount'
])
```
...
```html
<p>
  已选 <span>{{ checkedTotalCount }}</span> 件商品，总价：
...
```js
...mapGetters('cart', [
  'checkedTotalCount', 'checkedTotalPrice'
])
```
